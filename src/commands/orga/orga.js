...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("orga")
        .setDescription("Commande pour gérer les organisateurs dans sa fête (sa catégorie) !")
    ...
    async execute(interaction) {
        ...
        if (member === interaction.member) return interaction.reply({ content: "Vous êtes déjà l'organisateur de cette fête !", ephemeral: true });
        if (member.user.bot) return interaction.reply({ content: "Vous ne pouvez pas mettre un bot discord en tant qu'organisateur de votre fête !", ephemeral: true });
        ...
        if (!party || (party.organizer_id !== interaction.member.id && !interaction.member.permissions.has(PermissionFlagsBits.Administrator))) {
            return interaction.reply({
                content: "Tu dois être l'organisateur de cette fête (de cette catégorie) pour pouvoir gérer les organisateurs !" +
                "\nSi tu es organisateur et que tu veux gérer tes organisateurs, tape cette commande dans la catégorie de ta fête.",
                ephemeral: true,
            });
        }

        if (!party.guest_list_id.includes(member.id)) return interaction.reply({ content: `Vous devez d'abord ajouter ${member} en tant qu'invité à votre fête avant de l'ajouter en tant qu'organisateur !`, ephemeral: true });
        ...
        switch (interaction.options.getSubcommand()) {
             ...
             */
            case "ajouter":
                if (party.organizer_list_id.includes(member.id)) return interaction.reply({ content: `${member} est déjà sur votre liste d'organisateur à votre fête !`, ephemeral: true });
                ...
                try {
                    ...
                    return interaction.reply({ content: "Une erreur est survenue lors de l'ajout de l'organisateur à votre fête !", ephemeral: true });
                }
                ...
                return interaction.reply({ content: `${member} a bien été ajouté sur votre liste d'organisateur pour votre fête !`, ephemeral: true });

            /**
             * Remove a member from the party as a organizer
             */
            case "retirer":
                if (!party.organizer_list_id.includes(member.id)) return interaction.reply({ content: `${member} n'est déjà pas sur votre liste des organisateurs à ta fête !`, ephemeral: true });
                ...
                try {
                    ...
                    return interaction.reply({ content: "Une erreur est survenue lors de la suppression de l'organisateur à votre fête !", ephemeral: true });
                }

                return interaction.reply({ content: `${member} a bien été retiré de votre liste d'organisateur pour votre fête !`, ephemeral: true });
        }
