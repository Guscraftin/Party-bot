    ...
    async execute(interaction) {
        ...
        await interaction.guild.members.fetch(membreId)
            .then(async function(membre) {
                ...
                await membre.send("A la suite de votre demande pour changer votre pseudo, celle-ci a été __refusé__.\n" +
                    "En effet, **votre pseudo doit commencer par votre `Prénom`**.\n" +
                    `De ce fait, il reste \`${membre.nickname}\` et ne change pas en \`${newName}\` !\n\n` +
